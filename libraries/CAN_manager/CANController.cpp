...
uint8_t* CANControllerClass::getRXframe(){
	return _rxData;
}
...
void CANControllerClass::readFrame ( long* Id, bool * Extended,int* Dlc,uint8_t* rx_frame_datas){
	*Id=_rxId;
	*Extended=_rxExtended;
	*Dlc=_rxDlc;
	for (int i=0;i<_rxDlc;i++)
	{
		rx_frame_datas[i]=_rxData[i];
	}
}
