...
int MCP2515Class::begin(long baudRate)
{
  ...
  const struct {
    ...
    { (long)16E6,  (long)400E3, { 0x00, 0xBA, 0x07 } },
...
void MCP2515Class::sendPacket(uint8_t* message,int id, int dlc , bool rtr ){
	beginPacket(id,dlc,rtr);
	write(message,_txDlc);
	endPacket();
}

void MCP2515Class::sendExtendedPacket(uint8_t* message,long id, int dlc , bool rtr ){
